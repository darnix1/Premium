...
function vmess() {
    ...
    for db1 in "${vm[@]}"; do
        ...
        while IFS= read -r a; do
            if [[ -n ${a} ]]; then
                ...
                nowt=$((now - client))
                if [[ ${nowt} -lt 40 ]]; then
                    if ! grep -qw "${ina}.*${enu}" /tmp/vm; then
    ...
    if [[ -s /tmp/vm ]]; then
        for vmuser in "${vm[@]}"; do
            vmhas=$(grep -wc "${vmuser}" /tmp/vm)
            vmhas2=$(grep -w "${vmuser}" /tmp/vm | cut -d ' ' -f 2-8 | nl -s '. ' | sed 's/^/├─ /')
...
function vless() {
    ...
    for db2 in "${vldat[@]}"; do
        ...
        while IFS= read -r a; do
            if [[ -n ${a} ]]; then
                ...
                nowt=$((now - client))
                if [[ ${nowt} -lt 40 ]]; then
                    if ! grep -qw "${ina}.*${enu}" /tmp/vl; then
    ...
    if [[ -s /tmp/vl ]]; then
        for vlus in "${vldat[@]}"; do
            vlsss=$(grep -wc "${vlus}" /tmp/vl)
            vlsss2=$(grep -w "${vlus}" /tmp/vl | cut -d ' ' -f 2-8 | nl -s '. ' | sed 's/^/├─ /')
